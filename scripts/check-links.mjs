...
for (const href of requiredExternal) {
  if (!externalLinks.has(href)) failures.push(`missing required external link: ${href}`);
}

const requiredMailto = ['mailto:hello@registrystack.org'];
...
for (const href of requiredMailto) {
  ...
  if (!mailtoAddresses.has(address)) failures.push(`missing required contact link: ${href}`);
}
